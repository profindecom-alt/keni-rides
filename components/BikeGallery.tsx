...
interface BikeGalleryProps {
  ...
  labels: string[];
  /**
   * Localised alt text aligned to images order. Kept separate from `labels`
   * because the tab controls want a short label ("Profil") while the image
   * needs the descriptive, keyword-bearing sentence that Google Images reads.
   */
  alts: string[];
  zoomLabel: string;
  closeLabel: string;
...
export default function BikeGallery({ images, name, labels, alts, zoomLabel, closeLabel }: BikeGalleryProps) {
  ...
  return (
    <div className="bike-gallery">
      ...
      >
        <img src={images[active]} alt={alts[active] ?? name} />
      ...
      {lightbox ? (
        <div className="lightbox open" onClick={() => setLightbox(false)} role="dialog" aria-modal="true" aria-label={name}>
          ...
          <img src={images[active]} alt={alts[active] ?? name} onClick={(e) => e.stopPropagation()} />
